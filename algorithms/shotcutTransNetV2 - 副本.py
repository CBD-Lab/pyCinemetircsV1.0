...
def getFrame_number(scenes):
    """
    接受内存中的场景数据（列表或 NumPy 数组），解析出帧编号。
    :param scenes: 场景数据，列表或 NumPy 数组，每个元素包含帧信息。
    :return: 提取的帧编号列表。
    """
    Frame_number = []
    ...
    # 遍历场景数据，提取第二列的帧编号
    for scene in scenes:
        if isinstance(scene, (list, tuple, np.ndarray)):
            Frame_number.append(scene[1])  # 假设每行有多个元素，第二列为帧编号
        else:
            raise ValueError(f"无效的场景数据格式: {scene}")
